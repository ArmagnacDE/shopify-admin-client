...
//
// Transport-Ebene: Token-Tausch, Timeout, Retry, Rate-Limit-Handling. Die Store-
// Registry (store-registry.js) reicht `fetch` bis hierher durch; der Barrel index.js
// re-exportiert createShopifyClient/clientFromEnv/isMutation unveraendert.
...
// Exportiert, damit die Registry dieselbe Pruefung fuer `expectedDomain` nutzt
...
function sanitizeLabel(s, max = 80) {
  const clean = Array.from(String(s), (ch) => {
 ...
 * @param {typeof fetch} [opts.fetch] Transport injizierbar (Registry/Tests). Wird ZUR
 *        AUFRUFZEIT aufgeloest — ohne Wert faellt jeder Request auf das aktuelle
 *        globalThis.fetch zurueck, NICHT auf einen beim Bau eingefrorenen Verweis.
...
export function createShopifyClient({
  ...
  if (fetchImpl !== undefined && typeof fetchImpl !== "function") {
    throw new Error(`Shopify-Client "${name}": fetch muss eine Funktion sein, wenn gesetzt.`);
  }
  ...
  async function fetchWithTimeoutRetry(url, options, retries) {
    for (let attempt = 0; ; attempt++) {
      try {
        // Aufrufzeit-Aufloesung: injizierter fetch (Registry/Tests) sonst der AKTUELLE
        // globalThis.fetch — ein Test darf globalThis.fetch nach dem Bau des Clients
        // tauschen (S0-A2), ohne dass ein eingefrorener Default das unterlaeuft.
        const doFetch = fetchImpl ?? globalThis.fetch;
        return await doFetch(url, { ...options, signal: AbortSignal.timeout(timeoutMs) });
      } catch (err) {
 ...
 *
 * ⚠️ UNGEGUARDET: liest `${prefix}_STORE` aus der Umgebung — der Endpoint ist damit ein
 * Env-Wert, nicht eine Code-Konstante. Fuer Firmen-Repos (armagnac, Wagemut) NICHT
 * benutzen: dort ist die Store-Registry (createStoreRegistry) mit `expectedDomain` als
 * Code-Konstante der Weg. `clientFromEnv` bleibt nur fuer Standalone-/Ad-hoc-Nutzung im
 * Paket; der Boundary-Scan (S2) verbietet seine Nutzung in Firmen-Repos.
 *
 * @param {string} prefix
 * @param {{ version?: string, label?: string, timeoutMs?: number, fetch?: typeof fetch }} [opts]
 */
export function clientFromEnv(prefix, { version, label, timeoutMs, fetch: fetchImpl } = {}) {
